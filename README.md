...
Keyless auth needs a Ryvn CLI with the OIDC credential source; there is no separate login step — every `ryvn` invocation exchanges the job's GitHub OIDC token on its own (cached within the process). Both the action and the reusable workflow (`.github/workflows/release.yml`) install the CLI and accept a `ryvn_cli_version` input (e.g. `v1.294.0`) to pin a release; when unset the action installs `v1.294.0`, the oldest release carrying the publishing commands for every registry provider (`create/delete registry-config` with brokered credentials, `package chart`, `push chart`, `create release-file`, `describe image`), the destination-repository preflight, and the image digest in `describe image -o artifact`. An older explicit pin fails at the first missing command with the CLI's own `unknown command` error. A CLI that predates keyless auth ignores `id-token: write` and uses the static credentials if set; otherwise its first `ryvn` call fails on missing credentials.
...
```bash
ryvn create registry-config "$RUNNER_TEMP/ryvn-auth" --service api -o env > auth.env && . ./auth.env   # brokered (GAR / customer ECR / generic); managed ryvn-registry ECR keeps aws ecr get-login-password
...
The bound registry decides how the action authenticates; it is never derived from the organization name or from the image URL.

| Bound registry | Login |
|---|---|
| `ryvn-registry` of type `elasticContainerRegistry` (Ryvn-managed ECR) | Assumes the org's GitHub Actions role and uses `amazon-ecr-login`, as before. `ryvn push chart --registry-config $DOCKER_CONFIG/config.json` reads that same Docker login, so no separate `helm registry login` is needed and a stale `HELM_REGISTRY_CONFIG` in the job cannot shadow it |
| Any other registry — managed `ryvn-registry` (GAR), a customer ECR, or a generic OCI registry (GHCR, Docker Hub, ...) | `ryvn create registry-config <dir> --service <name>` asks the hub to broker a credential for whatever registry the service is bound to and writes a disposable Docker/Helm config; the action writes it inside its own per-invocation temp workspace, exports its `DOCKER_CONFIG`/`HELM_REGISTRY_CONFIG` for the build, push, and inspect steps, and an `always()` cleanup step restores the previous variables and removes the whole workspace (credentials plus the `.token_seed*` files Docker drops into `$DOCKER_CONFIG` during a build); a failed removal fails the step. The caller's own Docker/Helm/buildx directories are never touched. `ryvn delete registry-config` is for configs placed in a shared directory (see the custom-CI example) |

Before writing config files the CLI probes `GET /v2/<repository>/tags/list?n=1` on the destination: a missing repository fails the "Log in to registry" step with `repository <repo> does not exist in registry <name> (<host>); create it before building` (Ryvn never creates repositories in a customer registry). Credentials that can push but not list (401/403 on the probe) only warn. `--skip-repository-check` opts out via a custom-CI flow. If Ryvn refuses to issue credentials (missing `service.build`/`registry.publish`, publisher access not provisioned yet, or no publish credential on a customer registry), the action fails; it never falls back to AWS or anonymous pushes.
**Authorization:** `registry.read` is needed only on the managed `ryvn-registry` (read to route the AWS-role path) and — inside `create registry-config` — on a customer registry only while the repository preflight runs (`--skip-repository-check` opts out). The orchestrator reconciles `registry.read`/`registry.publish` onto the keyless CI identity for every registry a repository's services publish to (`ryvn-orchestrator` >= 0.896.1), so no manual grant is needed.

Brokered-credential notes:

- Managed-GAR credentials live for one hour and are issued immediately before the build-and-push step; customer-ECR tokens follow AWS's own expiry and generic credentials simply keep working until the customer rotates them. A single build whose build-and-push phase outlives the credential will still fail at push time; split long builds or use `build_only` plus a separate publish job.
- `create registry-config` seeds the disposable Docker config from the caller's (buildx builders, other registries and their `credsStore`/`credHelpers` stay as they are), pins only the destination host to file-backed auth so the token is never handed to a credential helper that would keep it after cleanup, and preserves `DOCKER_CERT_PATH` so Docker TLS still finds the caller's certificates. The buildx builder is created before the switch to the isolated config, so `docker/setup-buildx-action`'s post-job cleanup still finds it. The password never reaches action logs or outputs.
